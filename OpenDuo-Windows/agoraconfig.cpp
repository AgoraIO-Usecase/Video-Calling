...
{
    QDir iniPath(QString("AgoraConfigOpenDuo.ini"));
    if(!iniPath.exists()){
        QFile file(QString("AgoraConfigOpenDuo.ini"));
        file.open(QIODevice::ReadWrite);
    }
   m_spConfig = std::make_shared<QSettings>("AgoraConfigOpenDuo.ini",QSettings::IniFormat);
}
...
void CAgoraConfig::setCallingId(const QString &str)
{
    m_spConfig->setValue("/BaseInfo/identifier",str);
}

QString CAgoraConfig::getCallingId()
{
    QString id = m_spConfig->value("/BaseInfo/callingId").toString();
    if(id.isEmpty()){
        qsrand(QTime(0,0,0).secsTo(QTime::currentTime()));
        int number = qrand()%9999;
        id = QString("%1").arg(number,4,10,QLatin1Char('0'));
        m_spConfig->setValue("/BaseInfo/callingId", id);
    }
    return id;
}
